...
def sending_user(user_id,username,index):
  rowValues = work_sheet.row_values(index)
  data_row = rowValues[0].split('|')
  via_id = data_row[0]
  via_pass = data_row[1]
  via_2fa = data_row[2]
...
def handling_sending(
  ...
  message = sending_user(user_id,username,index)
  ...
  def apiMessages():
    if request.method =="POST":
      ...
      try: 
        ...
        print(user_text)
        dict_sheet = {key : value for key, value in enumerate(value_sheet) }
        list_sheet = []
        for key, value in dict_sheet.items():          
          print(key,value)

        # for data_value in value_sheet:
        #   print (data_value)
        #   handling_sending(
        #         user_id = user_id,
        #         username = username,
